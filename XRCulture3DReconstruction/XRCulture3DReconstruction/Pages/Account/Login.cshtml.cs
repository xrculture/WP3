...
namespace XRCulture3DReconstruction.Pages.Account
{
    [AllowAnonymous]
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            if (ModelState.IsValid && !string.IsNullOrEmpty(LoginInput.Username) && !string.IsNullOrEmpty(LoginInput.Password))
            {
                if (IsValidUser(LoginInput.Username, LoginInput.Password))
                {
                    ...
                    var claimsIdentity = new ClaimsIdentity(claims, "XRCulture3DReconstructionCookieAuth");
                    ...
                    await HttpContext.SignInAsync(
                        "XRCulture3DReconstructionCookieAuth",
